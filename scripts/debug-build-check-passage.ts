 ...
 *
 * OBSOLETE / HISTORICAL REPRO ONLY.
 *
 * This script targets the previous pre-binding testnet package and legacy
 * GatePolicy object. It is retained only for debugging history around the old
 * check_passage transaction builder and must not be used as active
 * FrontierWarden configuration. Current package/object truth lives in
 * scripts/testnet-addresses.json.
 *
 * Minimal isolated repro for legacy gate passage PTB construction.
 * No React. No providers. No signing. No submission.
 *
 * Usage:
 *   npx tsx scripts/debug-build-check-passage.ts
 *
 * NOTE: packageId below is deliberately legacy. Do not update this script for
 * current live operations without also replacing all object refs and retesting
 * the transaction builder against the fresh package.
 */
...
const ATTESTATION_REF = {
  objectId:
    "0x7733f5e5fa18892134afdeecd690c5cc607b4ed1f2fa0065238ccc16b83faece",
  version: "349181631",
...
async function main() {
  ...
  for (const c of coinsResult.data) {
    console.log(
      `      ${c.coinObjectId}  version=${c.version}  balance=${c.balance}`,
    );
  }
