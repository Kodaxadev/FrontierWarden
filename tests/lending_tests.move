...
module reputation::lending_tests {
    ...
    // Verifies lender receives BOTH the slashed vouch stake AND the collateral
    // (regression test for the P0 collateral-locked bug).
    #[test]
    fun test_slash_after_default_success() {
        ...
        // Both coins land in LENDER's inventory in the same tx.
        // Take them both at once to avoid re-taking a returned coin.
        // One must equal VOUCH_STAKE and the other COLLATERAL (regression:
        // before the P0 fix, collateral was sealed inside the Loan forever).
        test_scenario::next_tx(scenario, LENDER);
        {
            ...
            // Order is implementation-defined; verify both values are present.
            assert!(
                (v1 == VOUCH_STAKE && v2 == COLLATERAL) ||
                (v1 == COLLATERAL  && v2 == VOUCH_STAKE),
                2
            );
